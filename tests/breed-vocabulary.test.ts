...
const review = (over: Partial<ReviewRecord>): ReviewRecord =>
  ({
    ...
    review_fingerprint: "aaaaaaaaaaaa",
...
describe("vocabularyOf", () => {
  it("collects the distinct value of every heritable slot", () => {
    const vocab = vocabularyOf([
      review({}),
      review({ finder_model: "mistral-medium-latest", finder_provider: "mistral", skeptic_model: "mistral-medium-latest", skeptic_provider: "mistral" }),
  ...
  it("takes the newest fingerprint from reviewed_at, not from sorting it", () => {
    ...
    const vocab = vocabularyOf([
      review({ review_fingerprint: "ffffffffffff", reviewed_at: "2026-08-01T00:00:00+00:00" }),
      review({ review_fingerprint: "000000000000", reviewed_at: "2026-08-09T00:00:00+00:00" }),
    ]);
    expect(vocab.newestFingerprint).toBe("000000000000");
  });
  ...
  it("orders values deterministically, whatever order the reviews arrive in", () => {
    const records = [review({ finder_model: "mistral-medium-latest", finder_provider: "mistral" }), review({})];
